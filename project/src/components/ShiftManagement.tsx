...
import { Calendar, Clock, Users, ChevronLeft, ChevronRight, RefreshCw, Download, AlertTriangle, CalendarDays, BarChart3 } from 'lucide-react';
import { mockEmployees, shiftRequirements, mockLeaves } from '../data/mockData';
...
const ShiftManagement: React.FC = () => {
  ...
  const shiftAllocator = useMemo(() => new ShiftAllocator(employees, mockLeaves), [employees]);
  ...
  const regenerateSchedule = () => {
    alert('Schedule regenerated successfully with weekend coverage and leave replacements!');
  };

  const exportSchedule = () => {
    alert('Full week schedule (including weekends and leave coverage) exported successfully!');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-300">Manage 7-day schedules with weekend coverage, leave replacements, and alternating assignments</p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-gradient-to-br from-indigo-500/10 to-blue-600/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
          <div className="flex items-center justify-between mb-4">
        ...
        <div className="bg-gradient-to-br from-orange-500/10 to-amber-600/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20">
          <div className="flex items-center justify-between mb-4">
          ...
          <table className="w-full">
            <thead>
              <tr>
                ...
                {weekDates.map((date, index) => (
                  <th key={index} className={`text-center text-white font-semibold p-4 border-b border-white/20 min-w-[150px] ${isWeekendDate(date) ? 'bg-indigo-500/10' : ''}`}>
                    <div className="flex flex-col items-center">
                      <span>{formatDate(date)}</span>
                      {isWeekendDate(date) && (
                        <span className="text-xs text-indigo-400 mt-1">Weekend</span>
                      )}
                    </div>
                  </th>
                ))}
            ...
            <tbody>
              ...
              {weekdayShiftTypes.map((shiftType) => (
                <tr key={shiftType}>
                  ...
                  {weekDates.map((date, dateIndex) => {
                    ...
                    if (isWeekend) {
                      return (
                        <td key={dateIndex} className="p-4 border-b border-white/10 bg-indigo-500/5">
              ...
              {weekendShiftTypes.map((shiftType) => (
                <tr key={shiftType} className="bg-indigo-500/5">
                  <td className="p-4 border-b border-white/10">
                  ...
                  {weekDates.map((date, dateIndex) => {
                    ...
                    return (
                      <td key={dateIndex} className="p-4 border-b border-white/10 bg-indigo-500/5">
                        {shift ? (
                          <div 
                            className="p-3 rounded-lg border cursor-pointer transition-all duration-200 bg-indigo-500/10 border-indigo-500/30 hover:bg-indigo-500/20"
                            onClick={() => setSelectedShift(shift)}
                          >
                            <div className="flex items-center justify-between mb-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-slate-900 rounded-2xl p-6 max-w-lg w-full border border-white/20">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white mb-2 flex items-center">
                  ...
                  {selectedShift.isWeekend && (
                    <CalendarDays className="w-5 h-5 ml-2 text-indigo-400" />
                  )}
                </h2>
                <p className="text-gray-400">{selectedShift.date}</p>
                {selectedShift.isWeekend && (
                  <span className="inline-block px-2 py-1 text-xs font-medium bg-indigo-500/20 text-indigo-400 rounded-full mt-2">
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button className="px-6 py-2 bg-gradient-to-r from-teal-500 to-cyan-600 text-white rounded-lg hover:shadow-lg transition-all duration-200">
                Edit Shift
              </button>
            </div>
          </div>
