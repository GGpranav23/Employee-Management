import { Employee, Shift, ShiftType, WeekendShiftRecord, Leave } from '../types';
import { shiftRequirements } from '../data/mockData';

export class ShiftAllocator {
  private employees: Employee[];
  private leaves: Leave[];
  private weekendShiftCounter: Map<string, number> = new Map();
  
  constructor(employees: Employee[], leaves: Leave[] = []) {
  ...
  private isEmployeeOnLeave(employeeId: string, date: string): boolean {
  ...
  private getAvailableEmployees(date: string): Employee[] {
  ...
  private promoteKalpitaIfNeeded(availableEmployees: Employee[], requiredSeniors: number): Employee[] {
    const availableSeniors = availableEmployees.filter(emp => emp.skillLevel === 'Senior');
    
    if (availableSeniors.length < requiredSeniors) {
      const kalpita = availableEmployees.find(emp => emp.name === 'Kalpita' && emp.skillLevel === 'Junior');
      if (kalpita) {
        // Temporarily promote Kalpita to Senior for this allocation
        const modifiedEmployees = availableEmployees.map(emp => 
          emp.id === kalpita.id 
            ? { ...emp, skillLevel: 'Senior' as const }
            : emp
        );
  ...
  allocateShifts(weekStart: string): Shift[] {
    const shifts: Shift[] = [];
    const weekDates = this.getFullWeekDates(weekStart);
  ...
  private allocateWeekdayShifts(date: string): Shift[] {
    const shifts: Shift[] = [];
    let availableEmployees = this.getAvailableEmployees(date);
    ...
    for (const shiftType of weekdayShiftTypes) {
      ...
      // Apply Kalpita promotion logic if needed
      const modifiedEmployees = this.promoteKalpitaIfNeeded(availableEmployees, requirements.seniors);
  ...
  private getWeekendAvailableEmployees(date: string): Employee[] {
    return this.getAvailableEmployees(date);
  }
  ...
  findReplacement(originalEmployeeId: string, date: string, shiftType: ShiftType): string | null {
    const originalEmployee = this.employees.find(emp => emp.id === originalEmployeeId);
    if (!originalEmployee) return null;

    const availableEmployees = this.getAvailableEmployees(date);
    const sameSkillLevel = availableEmployees.filter(emp => 
      emp.skillLevel === originalEmployee.skillLevel && emp.id !== originalEmployeeId
    );

    if (sameSkillLevel.length > 0) {
      return sameSkillLevel[0].id;
    }

    // If no same skill level available, try Kalpita promotion for senior roles
    if (originalEmployee.skillLevel === 'Senior') {
      const kalpita = availableEmployees.find(emp => emp.name === 'Kalpita');
      if (kalpita) {
        return kalpita.id;
      }
    }
